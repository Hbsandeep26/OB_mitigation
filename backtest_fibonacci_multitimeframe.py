"""Backtest 5m-1m Multi-Timeframe Confirmed Fibonacci Pullback Options Strategy.

Detects structure and pullback levels on 5m candles, and triggers entries 
on 1m candles based on MA crossover, price-MA cross, candlestick reversals, 
or breakouts. Supports tight 1m stop-losses.
"""
...
class BacktestParams:
    ...
    trigger_type: str = "none"  # "none", "ma_crossover", "price_ma_cross", "bar_breakout", "candlestick"
    stop_loss_type: str = "5m_origin"  # "5m_origin", "1m_candle_low"
...
def setup_logging(output_dir: Path) -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(output_dir / "backtest_fibonacci_multitimeframe.log", encoding="utf-8"),
        ],
    )
...
def run_5m_state_tracker(df_5m: pd.DataFrame, params: BacktestParams) -> pd.DataFrame:
    """Track Fibonacci pullback legs on 5-minute candles."""
    L = params.pivot_len
    pivot_hi_vals, pivot_hi_idxs, pivot_lo_vals, pivot_lo_idxs = calculate_pivots(df_5m, L)
    
    trend_col = [0] * len(df_5m)
    anchor_hi_col = [0.0] * len(df_5m)
    anchor_lo_col = [0.0] * len(df_5m)
    zone_entry_col = [0.0] * len(df_5m)
    stop_loss_col = [0.0] * len(df_5m)
    take_profit_col = [0.0] * len(df_5m)
    
    trend = 0
    anchor_hi, anchor_lo = 0.0, 0.0
    ...
    for i in range(2 * L, len(df_5m)):
        ...
        if trend != 0:
            ...
            if trend == 1:
                if close_price < stop_loss or leg_age > params.max_leg_age:
                    trend = 0
                else:
                    leg_age += 1
            elif trend == -1:
        ...
        trend_col[i] = trend
        anchor_hi_col[i] = anchor_hi
        anchor_lo_col[i] = anchor_lo
    ...
    df_state = pd.DataFrame({
        "dt": df_5m["dt"],
        "trend": trend_col,
        "anchor_hi": anchor_hi_col,
        "anchor_lo": anchor_lo_col,
...
def compute_1m_triggers(df_1m: pd.DataFrame, trigger_type: str) -> tuple[pd.Series, pd.Series]:
    ...
    elif trigger_type == "price_ma_cross":
        ema20 = closes.ewm(span=20, adjust=False).mean()
        bull_trigger = (closes > ema20) & (closes.shift(1) <= ema20.shift(1))
        bear_trigger = (closes < ema20) & (closes.shift(1) >= ema20.shift(1))
        
    elif trigger_type == "bar_breakout":
        max3 = highs.rolling(3).max().shift(1)
        min3 = lows.rolling(3).min().shift(1)
        bull_trigger = closes > max3
        bear_trigger = closes < min3
        
    elif trigger_type == "candlestick":
        body = (closes - opens).abs()
        lower_wick = df_1m[["open", "close"]].min(axis=1) - lows
        upper_wick = highs - df_1m[["open", "close"]].max(axis=1)
        
        hammer = (lower_wick >= 2.0 * body) & (upper_wick <= 0.2 * body) & (body > 0)
        shooting_star = (upper_wick >= 2.0 * body) & (lower_wick <= 0.2 * body) & (body > 0)
        
        bull_engulfing = (closes > opens) & (closes.shift(1) < opens.shift(1)) & (closes >= opens.shift(1)) & (opens <= closes.shift(1))
        bear_engulfing = (closes < opens) & (closes.shift(1) > opens.shift(1)) & (closes <= opens.shift(1)) & (opens >= closes.shift(1))
        
        bull_trigger = hammer | bull_engulfing
        bear_trigger = shooting_star | bear_engulfing
        
    return bull_trigger, bear_trigger


def backtest_mtf(symbol: str, df_5m: pd.DataFrame, df_1m: pd.DataFrame, params: BacktestParams) -> list[Trade]:
    trades = []
    
    # 1. Run 5m Leg State tracker
    df_state = run_5m_state_tracker(df_5m, params)
    
    # 2. Align 5m leg states to 1m candles without lookahead bias
    ...
    for i in range(20, n):
        ...
        if trade_active:
            ...
            if exit_reason is not None:
                ...
                total_bars = 375  # Roughly max minutes in a session
                
                # Determine R:R target for option spread cap
                realized_rr_target = params.target_rr
                if params.target_type in ("extreme", "extension_0272"):
                ...
                if params.discrete_risk_budget is not None and params.discrete_risk_budget > 0:
                    if params.strategy_type == "Ratio":
                        ...
                        risk_budget_rupees = params.capital * 0.005
        ...
        if not trade_active and pending_pullback:
            ...
            if pending_direction == "BULLISH":
                if close_price < pending_stop_5m:
                    pending_pullback = False  # Invalidation
            else: # BEARISH
                if close_price > pending_stop_5m:
                    pending_pullback = False  # Invalidation
        ...
        if not trade_active:
            ...
            # Reset pending if 5m structure is flat
            if trend_5m == 0:
                pending_pullback = False
            ...
            if trend_5m != 0:
                ...
                # We check daily entry limits
                if params.entry_start <= time_str <= params.entry_cutoff:
                    # 1. Look for pullback touch to activate pending state
                    if not pending_pullback:
                        if trend_5m == 1 and low_price <= zone_entry_5m:
                            pending_pullback = True
                            pending_direction = "BULLISH"
                            pending_zone_entry = zone_entry_5m
                            pending_stop_5m = stop_5m
                            pending_target_5m = target_5m
                        elif trend_5m == -1 and high_price >= zone_entry_5m:
                            pending_pullback = True
                    ...
                    # 2. Monitor pending state for entry confirmations
                    if pending_pullback:
                        score_val = 70
                        ...
                        if pending_direction == "BULLISH":
                            if bull_triggers[i]:
                                ...
                                # Entry price is close of 1m confirming candle
                                ...
                                if params.stop_loss_type == "5m_origin":
                                    ...
                                    # stop just below min low of last 2 candles
...
def run_grid_optimization(selected: list[dict], args: argparse.Namespace, output_dir: Path):
    logging.info("Starting Multi-Timeframe Confirmation Grid Search optimization on %d symbols...", len(selected))
    
    triggers = ["none", "ma_crossover", "price_ma_cross", "bar_breakout", "candlestick"]
    ...
    for trig, sl_type, strat in itertools.product(triggers, stop_losses, strategies):
        ...
        for sym, (df_5m_f, df_1m_f) in data_dict.items():
            trades = backtest_mtf(sym, df_5m_f, df_1m_f, params)
    ...
    write_csv(
        output_dir / "fib_mtf_optimization.csv",
        opt_rows,
        ["trigger_type", "stop_loss_type", "strategy_type", "trades", "win_rate", "expectancy_rr", "total_rr", "max_drawdown_rr"]
    )
    logging.info("Multi-Timeframe grid results written to %s", output_dir / "fib_mtf_optimization.csv")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest 5m-1m Multi-Timeframe Confirmed Pullback Strategy.")
    ...
    parser.add_argument("--trigger", default="none", choices=["none", "ma_crossover", "price_ma_cross", "bar_breakout", "candlestick"])
    parser.add_argument("--stop-loss", default="5m_origin", choices=["5m_origin", "1m_candle_low", "1m_atr_1.5"])
    parser.add_argument("--strategy", default="Ratio", choices=["Naked ATM Buy", "Debit Spread", "Credit Spread", "Ratio"])
    parser.add_argument("--discrete-risk-budget", type=float, default=300.0)
...
def main() -> int:
    ...
    for instrument in selected:
        ...
        trades = backtest_mtf(sym, df_5m_filtered, df_1m_filtered, params)
        all_trades.extend(trades)
        row = summarize(sym, trades)
        summary_rows.append(row)
        
    trade_rows = [asdict(t) for t in all_trades]
    write_csv(output_dir / "fib_mtf_trades.csv", trade_rows, TRADE_FIELDS)
    write_csv(output_dir / "fib_mtf_summary.csv", summary_rows, ["symbol", "trades", "win_rate", "avg_rr", "expectancy_rr", "total_rr", "profit_factor", "max_drawdown_rr", "best_rr", "worst_rr"])
    
    portfolio = summarize("PORTFOLIO", all_trades)
    logging.info(
        "Multi-Timeframe Portfolio Summary: trades=%d win_rate=%.2f%% expectancy=%.2fR total=%.2fR max_dd=%.2fR",
